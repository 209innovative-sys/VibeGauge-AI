...
Write-Host "=== Confusion-AI App.tsx metric hardening ===" -ForegroundColor Cyan
...
if (-not (Test-Path $appPath)) {
  Write-Error "App.tsx not found at: $appPath"
  exit 1
}

# Backup current App.tsx
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$backupDir = Join-Path $rootPath ("backup_App_tsx_metrics_" + $timestamp)
...
Write-Host "Backed up existing App.tsx to: $backupDir" -ForegroundColor Green

# Write new App.tsx content
$appContent = @'
...
const App: React.FC = () => {
  ...
  const renderMetric = (label: string, metric?: Metric) => {
    ...
    if (!Number.isFinite(numericScore)) {
      // If backend didn’t send a valid number, just skip this metric
      return null;
    }
        ...
        <main className="grid gap-4 md:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
          ...
          <section className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              {error && (
                <span className="text-xs text-red-400 max-w-xs line-clamp-2">
                  {error}
                </span>
              )}
            </div>
...
Write-Host "=== Metric hardening complete. Restart Vite dev server if running. ===" -ForegroundColor Cyan
