param(
  ...
  # put your Vercel production domain here (used for default CORS_ORIGINS suggestion)
  [string]$VercelProdOrigin = "https://confusion-ai.vercel.app"
)

Write-Host "=== FIX: Backend online + CORS + /health + better OpenAI errors ===" -ForegroundColor Cyan
...
$backupDir = Join-Path $root ("backup_backend_fetchfix_" + $ts)
New-Item -ItemType Directory -Force -Path $backupDir | Out-Null

if (Test-Path $indexJs) {
  Copy-Item $indexJs (Join-Path $backupDir "index.js.bak") -Force
  Write-Host "Backup: $backupDir\index.js.bak" -ForegroundColor DarkGray
}

# Ensure backend/.env.example exists (NO secrets)
$envExample = @"
# Backend env (Render)
...
# Comma-separated allowed origins. You can include your Vercel prod + allow previews by suffix in code.
# Example:
# CORS_ORIGINS=https://confusion-ai.vercel.app
CORS_ORIGINS=$VercelProdOrigin

# Optional model overrides
OPENAI_MODEL_TEXT=gpt-4o-mini
OPENAI_MODEL_VISION=gpt-4o-mini
"@
Write-Utf8NoBom (Join-Path $backendPath ".env.example") $envExample
Write-Host "Wrote: backend/.env.example" -ForegroundColor Green

# Write hardened backend/index.js (CommonJS)
$indexContent = @"
'use strict';
...
Write-Utf8NoBom $indexJs $indexContent
Write-Host "✅ Wrote backend/index.js" -ForegroundColor Green

# Install deps (best effort)
Push-Location $backendPath
try {
  if (Test-Path (Join-Path $backendPath "package.json")) {
    Write-Host "Installing backend deps (best effort)..." -ForegroundColor Yellow
    npm install express cors multer dotenv morgan openai | Out-Host
  } else {
    Write-Host "WARNING: backend/package.json not found. Skipping npm install." -ForegroundColor Yellow
  }
} finally {
  Pop-Location
}
...
Write-Host "1) Commit + push" -ForegroundColor Yellow
Write-Host "2) Render: redeploy latest commit, ensure OPENAI_API_KEY is set" -ForegroundColor Yellow
Write-Host "3) Vercel: confirm VITE_API_BASE_URL=https://confusion-ai.onrender.com (Production) then redeploy" -ForegroundColor Yellow
Write-Host "4) Test: https://confusion-ai.onrender.com/health" -ForegroundColor Yellow
